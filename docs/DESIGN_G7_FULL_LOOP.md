...
1. ~~IOB clamp in the watch enact path~~ **DONE + REFINED to EXACT phone parity (2026-07-15)** — `loopCycle` now applies `min(rate, (maxBolus×2 − IOB)×2 + scheduledBasal)` floored at 0, identical to the phone's `additionalActiveInsulinClamp: iobHeadroom` into `DoseMath.recommendedTempBasal` (`DoseMath.swift:426`, `LoopDataManager.swift:2003`). The watch's `generateRecommendation` `.tempBasal` case does NOT pass the clamp, so we apply it after — mathematically identical to the phone. (Replaced the cruder hold-at-schedule.)
2. ~~`pumpSuspended` check in `loopCycle`~~ **DONE** — skips enact while `sessionSuspended` (manual suspend), re-loops on resume.
3. ~~Unify display/enact caps + decide the ceiling~~ **DONE + RESOLVED (2026-07-15): the cap now DERIVES FROM THERAPY MAX-BASAL** (Jeremy's ruling). `WatchPodLoanCoordinator.maxTempBasalRate` is computed from `settings.maximumBasalRatePerHour`; the pod proof limit (`PodProofController.tempBasalRateProofLimit`, now instance-settable) is programmed to match before each temp command. No more magic 3.0 TEMP-TEST-CAP, no revert-to-1.0 chore, no display/enact mismatch (cap == proof limit == the algorithm's own bound). Fallback 3.0 only until settings sync.
...
7. Delete the synthetic re-anchor (§1.1) and the effects restriction (§1.2) — effects restriction removed (P1 earlier). **RE-ANCHOR FINDING (2026-07-15): the premise was imprecise.** Verified: `WatchPredictionEngine.swift:320-334` (the `storeEntry` block) runs ONLY for MANUAL entries (`storeEntry:true`), where storing a user-entered BG as `wasUserEntered:true` at wall-clock is CORRECT. The G7 STREAM path (`WatchPredictionStore.swift:179`, `storeEntry:false`) SKIPS it and anchors the algorithm on the real stored series verbatim (`run()` line 349-352) — momentum/ICE are already clean; `manualBG` is display-only there. So there is NO stream-corrupting re-anchor to delete. The real convergence (§1.3) is a cosmetic refactor: give the stream a dedicated `predict(fromStore:)` entry point so it stops passing `newest.quantity` as a fake `manualBG` — this changes NO dosing behavior. **Deferred as a clean refactor (does not fix a bug; must not break manual entry).**
