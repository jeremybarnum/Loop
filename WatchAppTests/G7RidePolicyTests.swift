...
//  WatchAppTests
//
//  Ride-only (mute record §5, ported by content): while a sensor is adopted, no pending connect
//  of ours on the bond and no scan; join Dexcom's link when the OS reports it. Pinned as a pure
//  policy — the watch-daemon mechanism behind it is in G7RidePolicy's own doc comment.
//
...
final class G7RidePolicyTests: XCTestCase {
    ...
    func testJoinsDexcomsLinkExactlyWhenItComesUpOnTheAdoptedSensor() {
        ...
        XCTAssertFalse(G7RidePolicy.shouldJoin(rideOnly: false, connected: true, isAdoptedPeripheral: true, alreadyConnected: false), "switch off: the pending connect handles it")
    }
    ...
    func testRideOnlyIsTheWatchDefault() {
