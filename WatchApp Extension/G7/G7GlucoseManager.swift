...
final class G7GlucoseManager {
    ...
    func start() { client.startSoak() }
    func stop()  { client.stopSoak() }

    private func inject(mgdl: Int, at date: Date) {
        // The G7 emits one EGV per ~5 min; guard a duplicate within a window.
        if let last = lastInjected, date.timeIntervalSince(last) < 60 { return }
        lastInjected = date
        ...
        loopManager.glucoseStore.addGlucoseSamples([sample]) { [weak self] _ in
