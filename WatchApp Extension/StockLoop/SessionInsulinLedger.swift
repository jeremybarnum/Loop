...
public struct SessionInsulinLedger {
    ...
    /// Resolves the insulin model for a dose. The stores used to own a model provider;
    /// that moved out to the loop, so the ledger takes the same decision as a function and
    /// asks it per dose — which is also what makes a dose's own `insulinType` authoritative.
    public let insulinModel: (InsulinType?) -> InsulinModel
    ...
    public init(insulinModel: @escaping (InsulinType?) -> InsulinModel,
                longestEffectDuration: TimeInterval) {
        self.insulinModel = insulinModel
        self.longestEffectDuration = longestEffectDuration
    }

    /// The window every schedule must cover for these doses: from the earliest dose to the
    /// last one's full effect tail. Schedules arrive as repeating daily schedules but the
    /// math now takes absolute timelines, so each read projects the schedule across exactly
    /// the span its doses can touch.
    private var scheduleWindow: (start: Date, end: Date)? {
        guard let first = doses.map(\.startDate).min(),
              let last = doses.map(\.endDate).max() else { return nil }
        return (first, last.addingTimeInterval(longestEffectDuration))
    }

    /// The granted schedule as an absolute timeline. A loan holds ONE schedule for its whole
    /// life, so `.distantPast` is the literal truth: it was already in effect before these
    /// doses and never changed under them.
    private func basalTimeline(_ schedule: BasalRateSchedule, over window: (start: Date, end: Date)) -> [AbsoluteScheduleValue<Double>] {
        BasalRateSchedule.generateTimeline(schedules: [(date: .distantPast, schedule: schedule)],
    ...
    public mutating func recordEnact(_ dose: DoseEntry) {
        if dose.type != .bolus {
            ...
            where doses[index].type != .bolus
                ...
                // The `where` clause above guarantees an overlap, so the trim always yields a
                // dose; keeping the original on nil would leave two doses covering the same
                // minutes, which is the one thing this structure must never allow.
                if let trimmed = doses[index].trimmed(to: dose.startDate) {
                    doses[index] = trimmed
                }
            }
        }
    ...
    public func insulinOnBoard(at date: Date, basalSchedule: BasalRateSchedule) -> Double {
        ...
        guard let window = scheduleWindow else { return 0 }
        let timeline = doses
            .map { $0.simpleDose(with: insulinModel($0.insulinType)) }
            .annotated(with: basalTimeline(basalSchedule, over: window))
    ...
    public func glucoseEffects(insulinSensitivity: InsulinSensitivitySchedule,
        ...
        let trimmed = doses.compactMap { dose -> DoseEntry? in
        ...
        return trimmed
            .map { $0.simpleDose(with: insulinModel($0.insulinType)) }
            .annotated(with: basalTimeline(basalSchedule, over: window))
            .glucoseEffects(insulinSensitivityHistory: insulinSensitivity.quantitiesBetween(start: window.start, end: window.end),
                            from: start,
                            to: end)
    }
