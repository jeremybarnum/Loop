//
//  PodProofKitGuardTests.swift
//  OmniBLECoreTests
//
//  Input guards on PodProofController.setTempBasal. These run against an
...
class PodProofKitGuardTests: XCTestCase {

    private func setTempBasalError(rate: Double, duration: TimeInterval) -> Error? {
        let controller = PodProofController()
        let exp = expectation(description: "completion")
        var result: Result<PodProofStatus, Error>?
    ...
    func testRefusesSubThirtyMinuteDuration() {
        ...
        guard case PodProofError.tempBasalDurationNotSupported? = error else {
    ...
    func testAcceptsSupportedDuration() {
        ...
        guard case PodProofError.notPaired? = error else {
    ...
    func testOffGridRateSnapsUpPastCap() {
        ...
        guard case PodProofError.tempBasalExceedsProofLimit? = error else {
