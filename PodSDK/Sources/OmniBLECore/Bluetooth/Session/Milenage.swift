...
class Milenage {
    ...
    init(k: Data, sqn: Data, randParam: Data? = nil, auts: Data = Data(repeating: 0x00, count: 14), amf: Data = Milenage.MILENAGE_AMF) throws {
        ...
        let cipher = try AES(key: k.bytesArray, blockMode: ECB(), padding: .noPadding)

        let random = OmniRandomByteGenerator()
        rand = randParam ?? random.nextBytes(length: Milenage.KEY_SIZE)

        let opc = Data(try cipher.encrypt(Milenage.MILENAGE_OP.bytesArray)) ^ Milenage.MILENAGE_OP
        let randOpcEncrypted = Data(try cipher.encrypt((rand ^ opc).bytesArray))
        ...
        let resAk = Data(try cipher.encrypt(resAkInput.bytesArray)) ^ opc
        ...
        let macAFull = Data(try cipher.encrypt((Data(macAInput) ^ randOpcEncrypted).bytesArray)) ^ opc
