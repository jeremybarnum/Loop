...
class BluetoothManager: NSObject {
    ...
    override init() {
        ...
        managerQueue.sync {
            // Modified from LoopKit/OmniBLE for watchOS port: CoreBluetooth state
            // restoration (CBCentralManagerOptionRestoreIdentifierKey) is iOS-only.
            #if os(iOS)
...
extension BluetoothManager: CBCentralManagerDelegate {
    ...
    // Modified from LoopKit/OmniBLE for watchOS port: CoreBluetooth state
    // restoration is iOS-only; guarded so the delegate method is absent on watchOS.
    #if os(iOS)
