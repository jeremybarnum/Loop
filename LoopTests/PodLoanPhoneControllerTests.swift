...
final class PodLoanPhoneControllerTests: XCTestCase {
    ...
    func testLargeNegativeResidualWarnsButKeepsLooping() throws {
        ...
        // claim, so the books are wrong, not the body. Asserted on the URGENT channel (ruled
        // 2026-08-24, symmetric urgency): any audit breach must survive a Focus mode, while the
        // ACTION stays asymmetric — looping continues, as the no-open assertion above pins.
    ...
    // MARK: - Since-last-sync checkpoints (R32 closed 2026-08-27)
    ...
    /// +0.30 U of loan-total drift (the port line's overnight false-open of 2026-08-26 was
    /// +0.35) split across two windows of +0.15 each — the whole-loan framing opens the loop,
    /// the since-last-sync framing correctly stays silent. Window fills here are continuous
    /// (this line's estimator) but seconds long, so they perturb residuals by well under a
    /// milli-unit — margins to the ±0.20 band stay ≥0.05 U throughout.
    func testCheckpointedDriftWithinEachWindowStaysSilent() throws {
        let controller = makeController()
        let grant = try establishLoan(controller)   // audit base = 10.0 @ the takeover reading
...
extension PodLoanPhoneControllerTests {
    ...
    func testNormalLoanResidueIsAbsolvedNotRediscovered() throws {
        ...
        waitUntil(timeout: 5, "reclaim verified") { controller.state == .owner && !controller.isReclaimSettlingOnly }
    ...
    func testTheReclaimsOwnResyncNeverYieldsThePhone() throws {
        ...
        waitUntil(timeout: 5, "force landed") { controller.state == .owner && !MockPumpManager.testConnectionReleased }
        XCTAssertTrue(controller.isReclaimSettlingOnly, "settle open, not yet verified")
        ...
        connectionReady = true
        waitUntil(timeout: 8, "reclaim verified") { controller.state == .owner && !controller.isReclaimSettlingOnly }
        ...
        XCTAssertFalse(controller.isPodLoanedOut, "tile stays the phone's own")
    ...
    func testTheTileTapReachesReclaimFromTheVetoPosture() throws {
        ...
        XCTAssertTrue(controller.isPodLoanedOut, "the tile's tap predicate includes the yield → Reclaim Now")
        ...
        XCTAssertFalse(controller.isPodLoanedOut, "tile is the phone's own again")
        waitUntil(timeout: 5, "dosing resumed") {
            self.lock.lock(); defer { self.lock.unlock() }
            return self.pauseCalls.last == false
        }
