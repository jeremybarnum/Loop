//  NOT IN THE TEST TARGET ON THIS BRANCH — AND THIS IS A REAL COVERAGE GAP.
//
//  Every assertion here drives DoseMath's `recommendedTempBasal`, a pure function fed a
//  hand-built predicted curve. That entry point does not exist on next-dev: the algorithm
//  computes its own forecast from inputs, so a synthetic curve cannot be handed to it. The
//  suite cannot be ported by swapping its helper — the approach itself depends on the old API.
//
//  What is lost: our own assertions that maxBasal is a hard ceiling and that the active-insulin
//  clamp binds. The LIMITS are still enforced — by LoopAlgorithm, via maxBasalRate and
//  maxActiveInsulinMultiplier — but that is upstream's test coverage, not ours.
//
//  To restore: build inputs (glucose history, doses, schedules) that PRODUCE a flat-high
//  forecast, run LoopAlgorithm.run, and assert on the recommendation it returns. Until then the
//  gate does not cover watch dosing limits, and should not be read as if it does.
//
//
...
final class WatchDosingLimitsTests: XCTestCase {
    ...
    private let suspendThreshold = LoopQuantity(unit: .milligramsPerDeciliter, doubleValue: 67)
